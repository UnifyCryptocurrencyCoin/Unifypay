...
angular.module('copayApp.controllers').controller('preferencesBitpayServicesController',
  function($rootScope, $scope, $state, $timeout, $ionicHistory, bitpayAccountService, bitpayCardService, popupService, gettextCatalog) {
    ...
    $scope.removeAccount = function(account) {
      var title = gettextCatalog.getString('Remove UnifyPay Account?');
      var msg = gettextCatalog.getString('Removing your UnifyPay account will remove all associated UnifyPay account data from this device. Are you sure you would like to remove your UnifyPay Account ({{email}}) from this device?', {
        email: account.email
      });
    ...
    $scope.removeCard = function(card) {
      var title = gettextCatalog.getString('Remove UnifyPay Card?');
      var msg = gettextCatalog.getString('Are you sure you would like to remove your UnifyPay Card ({{lastFourDigits}}) from this device?', {
